...
async.waterfall([
    ...
    function(callback) {
        ...
        var client = new kafka.Client(config.kafka.zkConnString, config.kafka.clientId, config.kafka.zkOptions);
        config.kafka.producer = new Producer(client);
        config.kafka.producer.on("ready", function() {
            callback();
        });
    },
